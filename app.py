...
FEATURE_NAMES = [
    ...
    "subdomain_count", "uses_https", "has_port", "special_char_count", "domain_entropy"
]
...
def predict():
    try:
        ...
        if len(features) != 13:  # Expect 13 features
            logging.error(f"Feature count mismatch for {web_link}: got {len(features)}, expected 13")
        ...
        logging.error(f"🚨 Prediction error for {web_link}: {e}")
